<template>
  <div class="compiler-config">
    ...
    <div class="tabs">
      <button :class="{ active: tab === 'general' }" @click="tab = 'general'">GÃ©nÃ©ral</button>
      <button :class="{ active: tab === 'php' }" @click="tab = 'php'">PHP</button>
      <button :class="{ active: tab === 'server' }" @click="tab = 'server'">Serveur</button>
      <button :class="{ active: tab === 'security' }" @click="tab = 'security'">SÃ©curitÃ©</button>
      <button :class="{ active: tab === 'files' }" @click="tab = 'files'">Fichiers</button>
      <button :class="{ active: tab === 'database' }" @click="tab = 'database'">Base de donnÃ©es</button>
      <button :class="{ active: tab === 'updates' }" @click="tab = 'updates'">Mises Ã  jour</button>
      ...
      <button :class="{ active: tab === 'advanced' }" @click="tab = 'advanced'">AvancÃ©</button>
    </div>
    <div v-if="tab === 'general'" class="tab-content">
      <div class="form-group">
        <label>Nom de l'exÃ©cutable</label>
      ...
      <div class="form-group">
        <label>IcÃ´ne de l'exÃ©cutable (.ico)</label>
        <div class="input-with-button">
          <input :value="store.currentProject?.iconPath" type="text" placeholder="Par dÃ©faut (Logo ExeOutput)" readonly />
    ...
    <div v-if="tab === 'files'" class="tab-content">
      <div class="files-selector">
        ...
        <p class="description">Choisissez les dossiers Ã  sortir de l'exÃ©cutable pour optimiser la taille ou la rapiditÃ©.</p>
    ...
    <div v-if="tab === 'database' && store.currentProject?.database" class="tab-content">
      <div class="db-config">
        <h3>Configuration de la Base de DonnÃ©es</h3>
        <p class="description">IntÃ©grez une base de donnÃ©es directement dans votre exÃ©cutable.</p>
        
        <div class="form-group">
          <label>Moteur de donnÃ©es</label>
          <select v-model="store.currentProject!.database!.type" @change="store.updateProject({ database: store.currentProject!.database })">
            ...
            <option value="sqlite">SQLite (LÃ©ger, un seul fichier)</option>
            <option value="mariadb">MariaDB Portable (Serveur autonome intÃ©grÃ©)</option>
          </select>
        </div>
        ...
        <div v-if="store.currentProject.database.type === 'mariadb'" class="mariadb-settings">
          ...
          <div class="form-group">
            ...
            <div class="input-with-button">
              <input :value="store.currentProject.database.initSqlPath" type="text" placeholder="Selectionnez un fichier .sql pour importer des donnÃ©es au 1er lancement" readonly />
              <button @click="selectInitSql">Parcourir</button>
            </div>
          </div>
          
          <div class="info-box warning">
            <p><strong>Note :</strong> L'exÃ©cutable cherchera <code>mysqld.exe</code> dans le dossier <code>data/mysql</code> lors du lancement.</p>
          </div>
        </div>
    ...
    <div v-if="tab === 'updates'" class="tab-content">
      <div class="info-box info">
        <p>Afin que vos exÃ©cutables finaux puissent se mettre Ã  jour tout seuls chez vos clients, vous devez fournir une URL pointant vers un fichier JSON (ex: version.json).</p>
      </div>
      <div class="form-group">
        <label>URL du fichier JSON de mise Ã  jour (Optionnel)</label>
        ...
        <small class="hint">Laissez vide pour dÃ©sactiver la mise Ã  jour automatique.</small>
      </div>
      <div class="form-group">
        <label>Notes de cette version (AffichÃ©es Ã  l'utilisateur)</label>
        <textarea v-model="store.currentProject!.notes" placeholder="Ex: Correction de bugs mineurs, AmÃ©lioration de l'interface..." @change="store.updateProject({ notes: store.currentProject!.notes })"></textarea>
      </div>
    </div>
    <div v-if="tab === 'server_config' && store.currentProject" class="tab-content">
      <h3>Configuration du dÃ©ploiement SFTP</h3>
      <p class="description">Configurez les accÃ¨s pour envoyer automatiquement vos fichiers sur votre serveur.</p>
      
      <div class="form-row">
        <div class="form-group flex-2">
          <label>HÃ´te SSH/SFTP</label>
      ...
      <div class="form-group">
        ...
        <small class="hint">Le dossier oÃ¹ seront dÃ©posÃ©s le .exe et le .json</small>
      </div>
      </div>
    </div>
    <div v-if="tab === 'environment' && store.currentProject" class="tab-content">
      <h3>Variables d'Environnement Externes</h3>
      <p class="description">DÃ©finissez ici les variables qui seront mises dans le fichier <code>.env.template</code> pour chaque station (ex: DB_HOST, STATION_NAME).</p>
      ...
      <div class="env-vars-editor">
        <div v-for="(val, key) in store.currentProject.envVars" :key="key" class="env-var-row">
          ...
          <span>=</span>
          <input :value="val" type="text" placeholder="Valeur par dÃ©faut" @change="updateEnvValue(key, $event)" />
          <button class="remove-btn" @click="removeEnvVar(key)">Ã—</button>
        </div>
        <button class="add-btn" @click="addEnvVar">+ Ajouter une variable</button>
      </div>

      <div class="info-box info" style="margin-top: 2rem;">
        <p><strong>Note :</strong> Ces variables seront Ã©ditables directement sur chaque poste via l'interface de configuration de l'application (Ctrl+Shift+S).</p>
      </div>
    </div>
...
// Suggestion automatique de l'URL de mise Ã  jour
...
const selectIcon = async () => {
  if (!store.currentProject) return;
  const selected = await dialog.open({
    filters: [{ name: 'IcÃ´nes Windows', extensions: ['ico'] }],
    multiple: false,
  });
  if (selected) {
...
const save = async () => {
  ...
  try {
    ...
    compilerStore.addLog('info', 'Projet enregistrÃ© avec succÃ¨s !');
